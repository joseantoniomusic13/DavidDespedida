...
function verificarCodigo() {
    ...
    // ── 1.5. Contraseña maestra Platino: uso ilimitado ──
    if (codigoUpper === LLAVE_MAESTRA_PLATINO.toUpperCase()) {
        llavePlatinoPendiente = LLAVE_MAESTRA_PLATINO;
        retoComodin = null;
        cerrarModalLlave();
        setTimeout(() => {
            mostrarModalLlavePlatino(LLAVE_MAESTRA_PLATINO);
        }, 300);
        return;
    }
...
function ejecutarRetoPuede() {
    ...
    historialRetos.push({ texto: reto.texto, emoji: reto.emoji, color: reto.color, ronda: rondaActual, completado: true, puntos: puntos });
    guardarEstado();
    actualizarHistorial();
    actualizarRetosRestantes();
...
function actualizarHistorial() {
    ...
    historialRetos.forEach((entry, i) => {
        ...
        const puntos = entry.puntos ? `<span style="margin-left: auto; color: ${color}; font-weight: bold; font-family: var(--font-display); font-size: 0.9rem; padding-right: 15px; letter-spacing: 1px;">+${entry.puntos} CS</span>` : '';
...
function desbloquearLogro(id) {
    if (!logrosDesbloqueados.includes(id)) {
        ...
        if (el) {
            ...
            // Extraer nombre e icono de la medalla
            const icon = el.querySelector('.medalla-icon').textContent;
            const titulo = el.querySelector('h4').textContent;

            // Efecto visual/sonoro
            SoundFX.confetti();

            // Mostrar notificación tipo "Logro Desbloqueado"
            setTimeout(() => {
                ...
                toast.style.cssText = `
                    ...
                    background: linear-gradient(135deg, rgba(23, 23, 26, 0.95), rgba(30, 30, 40, 0.95));
                    border: 1.5px solid #e4ae39;
                    border-radius: 12px;
                    padding: 1rem 1.5rem;
                    display: flex;
                    align-items: center;
                    gap: 1rem;
                    z-index: 10000;
                    box-shadow: 0 10px 30px rgba(0,0,0,0.8), 0 0 20px rgba(228, 174, 57, 0.4);
                ...
                toast.innerHTML = `
                    <div style="font-size: 2.2rem; filter: drop-shadow(0 0 10px rgba(228, 174, 57, 0.5));">${icon}</div>
                    <div style="display: flex; flex-direction: column;">
                        <span style="color: #e4ae39; font-size: 0.75rem; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 0.2rem;">¡Medalla de Servicio!</span>
                        <span style="color: #fff; font-size: 1.1rem; font-weight: 600; text-shadow: 0 2px 4px rgba(0,0,0,0.5);">${titulo}</span>
                    </div>
                `;
...
// RETO EXPRESS
// ============================================================
function abrirEscribirExpress() {
    document.getElementById('express-input-overlay').classList.add('visible');
    document.getElementById('express-input-text').value = '';
    document.getElementById('express-input-text').focus();
}

function cerrarEscribirExpress() {
    document.getElementById('express-input-overlay').classList.remove('visible');
}

function lanzarRetoExpress() {
    const texto = document.getElementById('express-input-text').value.trim();
    if (!texto) return;

    // Cerrar input
    cerrarEscribirExpress();

    // Configurar texto
    document.getElementById('express-error-texto').textContent = texto;

    // Sonido de error/glitch
    if (typeof SoundFX !== 'undefined') {
        SoundFX.flash();
        setTimeout(() => SoundFX.tick(0.1), 100);
        setTimeout(() => SoundFX.tick(0.1), 300);
    }
    // Mostrar error
    document.getElementById('express-error-overlay').classList.add('visible');
}

function aceptarRetoExpress() {
    const texto = document.getElementById('express-error-texto').textContent;
    ...
    historialRetos.push({
        ...
        emoji: "⚡",
        color: "#ef4444",
        ronda: rondaActual || 'express',
        completado: true,
        puntos: 3000
    });

    // Sumar 3000 puntos al completar el Reto Express
    const oldRating = csRating;
    csRating += 3000;
    actualizarRatingUI();
    comprobarRankUp(oldRating, csRating);
...
function cerrarErrorExpress() {
    document.getElementById('express-error-overlay').classList.remove('visible');
}

function negarseRetoExpress() {
    cerrarErrorExpress();
    // Mostrar modal visual de castigo
    document.getElementById('express-denied-overlay').classList.add('visible');
}

function cerrarDeniedExpress() {
    document.getElementById('express-denied-overlay').classList.remove('visible');
}

// ============================================================
// COMPARTIR PROGRESO POR WHATSAPP
// ============================================================
function compartirProgresoWhatsApp() {
    const totalCompletados = historialRetos.length;
    let ultimoReto = "Todavía no ha completado ningún reto.";
    if (totalCompletados > 0) {
        ultimoReto = historialRetos[historialRetos.length - 1].texto;
    }

    let mensaje = `🔥 *REPORTE DE LA DESPEDIDA DE DAVID* 🔥\n\n`;
    mensaje += `🏆 *Rango CS2 actual:* ${csRating}\n`;
    mensaje += `✅ *Retos completados:* ${totalCompletados}\n`;
    mensaje += `🔑 *Llaves usadas:* ${llavesUsadas.length}\n`;

    if (totalCompletados > 0) {
        mensaje += `\n🎯 *Último reto completado:*\n"${ultimoReto}"\n`;
    }
